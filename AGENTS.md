...
- `localStorage` persistence key `pickleball-rr-session-v1` — saves in-progress tournament state (`roundsData`, `scoresData`, `courtsData`, `currentRound`, `totalRounds`, `playersData`, `skillsData`, `sessionId`); restored on page reload so a mobile refresh on a court post does not lose recorded scores. `generate()` overwrites the session (assigning a fresh `sessionId`); `goBack()` preserves it.
- `localStorage` persistence key `pickleball-rr-roster-v1` — persistent **lifetime** stats, independent of the per-day session. Shape: `{ v, players, applied }` where `players` is keyed by normalized name (`name.trim().toLowerCase()`) holding `{ name, skill, gw, gl, pw, pl }` (games won/lost, points won/lost), and `applied` is a reconcile ledger mapping each `sessionId` to the per-player delta last folded in. `applySession(roster, id, deltas, meta)` does `roster += (new − previouslyApplied[id])`, so saving/importing the same id is **idempotent** (corrects rather than double-counts). Placeholder names (`/^player\s*\d+$/i`) and unscored players are never recorded.
- **Overall tab** (`#tabOverall`, 4th bottom tab): `refreshOverall()` renders all-time standings from the roster (same columns/sort as the live Scoreboard). "Save Today's Results to Overall" (`saveToOverall()`, also on the complete banner) folds the current session's `computeStandings()` deltas into the roster.
- **Export / Import** (Overall header): `exportData()` emits the current day's results as a minified bundle `{t:"pkbl-rr",v,id,d,p:[{n,s,gw,gl,pw,pl}]}` (copied to clipboard + shown for manual copy); `confirmImport()` parses a pasted bundle and `applySession()`s it (additive merge by name, idempotent by `id`). Designed so a stand-in can text the host one day's results to fold in.
- **Player-name autocomplete** (`setupAutocomplete()`): a single shared `#acMenu` dropdown, repositioned under the focused `p1`–`p8` input, suggests roster names; picking one fills the name and sets the matching `sk{i}` skill dot.
